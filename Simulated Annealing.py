...
def simulated_annealing():
    current_route = createInitialRoute()
    current_distance = Distace(current_route)
    best_route = current_route.copy()
    best_distance = current_distance
    temperature = INITIAL_TEMP

    for _ in range(MAX_ITERATION):
        neighbors = GetNeighbors(current_route)
        next_route = random.choice(neighbors)
        next_distance = Distace(next_route)
...
solution1 , cost2 = simulated_annealing()
print(solution1 , cost2)